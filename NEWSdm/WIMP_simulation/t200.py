...
N_ion = 40000 # total n_wimps to be recoiled
E_ion = 3.2e5 # eV
theta_ion = 0 # degrees
ion_name = 'Ag'
# n_bins = {'E':50, 'theta':30} # N_ex2N_theta binning of E-theta spectrum
# M_w = 100 # GeV
slots = 200 # n_runs of SRIM
jobs = 5 # n_jobs for Parallel
...
def parallel_tracks(n_ion=1000, E_ion=1e5, theta_ion=0, ion_name='C', layer=None, srim_dir=None, output_dir=None, track_3d=None):
    ...
    ion = srim.Ion(ion_name, energy=E_ion) #eV
...
paral_params = dict(E_ion=E_ion, theta_ion=theta_ion, ion_name=ion_name, layer=layer, srim_dir=srim_dir, output_dir=output_dir, track_3d=track_3d)
tracks_for_N = Parallel(n_jobs=jobs, verbose=1)(delayed(parallel_tracks)(n, **paral_params) for n in [N_ion//(jobs*slots) for i in range(jobs*slots) ] )
...
# del trim, target, ion; gc.collect()
np.savetxt(output_dir+'/tracks_'+ion_name+str(int(E_ion/1000))+'.csv', tracks_ion, delimiter=',')
