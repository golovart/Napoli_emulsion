...
for detec in ['emul', 'gas', 'cryst']:
    for name,el in elems[detec].items():
        elems[detec][name]['c_n'] = np.round(el['st_n']*el['m_n']/np.sum([el['st_n']*el['m_n'] for el in elems[detec].values()]),3)
...
for detec in ['emul', 'gas', 'cryst']:
    os.makedirs(detec, exist_ok=True)
    ...
    with open(detec+'/ranges.txt','w',newline='') as frange:
    ...
    with open(detec+'/R_tot.txt','w') as f_tot:
        f_tot.write('R_tot (kg*d): {:e}'.format(whole_spec[M_w]))
    for name,el in elems[detec].items():
        np.savetxt(detec+'/spec_'+name+'.txt', spectrum[M_w][name], delimiter=',')
