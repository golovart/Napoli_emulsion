...
# sh -c "cp /opt/pysrim/srim_fix.py /usr/local/lib/python3.6/dist-packages/srim/srim.py; xvfb-run -a python3.6 /opt/pysrim/wimp_seq.py"
'''
cp /opt/pysrim/srim_fix.py /usr/local/lib/python3.6/dist-packages/srim/srim.py
pip3 install joblib
sh -c "xvfb-run -a python3.6 /opt/pysrim/wimp_seq.py"
'''
...
N_wimps = 10000 # total n_wimps to be recoiled
n_bins = {'E':20, 'theta':10} # N_ex2N_theta binning of E-theta spectrum
M_w = 100 # GeV
...
def energy_tracks(i_E, name, int_spec, n_bins=None, spec_Etheta=None, layer=None, srim_dir=None, output_dir=None, track_3d=None):
    ...
    for j in range(n_bins['theta']):
        ...
        tracks_theta = np.vstack((tracks_theta, proc_exyz(srim_tmp+'/SRIM Outputs/')))
    return tracks_theta
