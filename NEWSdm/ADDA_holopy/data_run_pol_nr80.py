...
# parameters
scan_name = 'ADDA/pol/NR80' # 'fog/Scan1'
...
def load_root_pol_images(imcheck, root_run, dr=20, n_pol=8, return_pols=False):
    """
    ...
    n_pols -- number of polarisations of each image
    ...
    pols = []
    imgs = np.zeros((imcheck.shape[0],dr*2,dr*2,n_pol), dtype=np.uint8)
    err_vid = set()
#     caput = StringIO()
#     sys.stdout = caput
    ...
    for ind, row in imcheck.iterrows():
        ...
        clust_im = np.zeros((dr*2,dr*2,n_pol), dtype=np.uint8)
        skip = False
        for i_pol in range(n_pol):
            ### GetGRIM(iv, igr, ipol, icol, r)
            im = root_run.GetGRIM(int(row['HeaderID']), int(row['GrainID']), i_pol, 0, dr) #arun.GetCLIMBFC(int(row['HeaderID']), int(row['pol'+str(i_pol)]), i_pol, dr, int(x0), int(y0) )
...
id_header = ['HeaderID','ViewID','GrainID','pol0','pol1','pol2','pol3','pol4','pol5','pol6','pol7','isolated','n_pol']
col_ids = pd.DataFrame(col_vgr, columns=id_header)
...
for id_part in tqdm(range(num_parts), desc='loading data part'):
    ...
    with stdout_redirector(f_redir):
        ims, pols, errs = load_root_pol_images(col_ids[id_part*part:(id_part+1)*part], arun, dr=dr, n_pol=n_pol, return_pols=True)
    with open(log_dir+'out_log_p'+str(id_part)+'.log','w') as fout:
        fout.write(f_redir.getvalue().decode('utf-8'))
    with h5py.File(out_data_dir+'data_raw_root_ims_pol_'+str(dr*2)+'.h5','a') as dfile:
