...
def identity_block_2d(X, f, filters, stage, block):
    """
    Arguments:
    X -- input tensor of shape (m, n_H_prev, n_W_prev, n_D_prev, n_C_prev)
    f -- integer, specifying the shape of the middle CONV's window for the main path
    ...
    # defining name basis
    conv_name_base = 'res_2d_' + str(stage) + block + '_branch'
    bn_name_base = 'bn_2d_' + str(stage) + block + '_branch'
    ...
    # Third component of main path (≈2 lines)
    #X = Conv3D(filters=F3, kernel_size=1, padding='valid', kernel_regularizer=regularizers.l2(1e-3), kernel_constraint=MaxNorm(4., axis=[0,1,2,3]), name=conv_name_base+'2c')(X)
    X = Conv2D(filters=F3, kernel_size=1, padding='same', kernel_constraint=MaxNorm(4., axis=[0,1,2]), name=conv_name_base+'2c')(X)
    X = BatchNormalization(axis=-1, name=bn_name_base+'2c')(X)
...
def convolutional_block_2d(X, f, filters, stage, block, s = 2):
    """
    Arguments:
    X -- input tensor of shape (m, n_H_prev, n_W_prev, n_D_prev, n_C_prev)
    f -- integer, specifying the shape of the middle CONV's window for the main path
    ...
    # Third component of main path (≈2 lines)
    #X = Conv3D(F3, 1, kernel_regularizer=regularizers.l2(1e-3), kernel_constraint=MaxNorm(4., axis=[0,1,2,3]), name=conv_name_base+'2c')(X)
    X = Conv2D(F3, 1, kernel_constraint=MaxNorm(4., axis=[0,1,2]), padding='same', name=conv_name_base+'2c')(X)
    X = BatchNormalization(axis=-1, name=bn_name_base+'2c')(X)

    ##### SHORTCUT PATH #### (≈2 lines)
    #X_shortcut = Conv3D(F3, 1, strides=s, kernel_regularizer=regularizers.l2(1e-3), kernel_constraint=MaxNorm(4., axis=[0,1,2,3]), name=conv_name_base+'1')(X_shortcut)
    X_shortcut = Conv2D(F3, 3, strides=s, kernel_constraint=MaxNorm(4., axis=[0,1,2]), padding='same', name=conv_name_base+'1')(X_shortcut)
    X_shortcut = BatchNormalization(axis=-1, name=bn_name_base+'1')(X_shortcut)
...
### FOR WAVELENGTH STUDY
def peak_lambda(img, b_g_eff, r_g_eff, lamb_range):
    ...
    return (lamb_range[np.argmin(np.sum((np.vstack((b_g_eff-peak_b_g, r_g_eff-peak_r_g)))**2, axis=0))]-np.min(lamb_range))/(np.max(lamb_range)-np.min(lamb_range))

# numpy-array approach
class Batch_data_generator_wave(keras.utils.Sequence):
    #images and labels are expected to be HDF5 datasets
    def __init__(self, images, labels, batch_size=256, shuff=True, multiclass=True, n_cl=5, wave_par=None):
        self.images, self.labels = images, labels
        self.batch_size = batch_size
        self.indices = np.arange(self.images.shape[0])
        self.batch_ids = np.arange(np.ceil(self.images.shape[0] / float(self.batch_size)), dtype=int)
    ...
    def __getitem__(self, idx):
        ...
        load_ids = list(self.indices[idx * self.batch_size:(idx + 1) * self.batch_size])
        batch_x = self.images[load_ids]
        peak_x = np.array([peak_lambda(img, **self.wave_par) for img in batch_x])
        #batch_x = batch_x/256.
        batch_y = self.labels[load_ids]
        if self.multiclass: batch_y = to_categorical(batch_y, num_classes=self.num_class)
        return [batch_x, peak_x], batch_y
        #return batch_x[...,np.newaxis], batch_y

class Batch_data_augmentator_wave(keras.utils.Sequence):
    #images and labels are expected to be HDF5 datasets
    def __init__(self, images, labels, batch_size=256, shuff=True, multiclass=True, n_cl=5, angles_list=None, wave_par=None):
        self.images, self.labels = images, labels
        self.batch_size = batch_size
        self.indices = np.arange(self.images.shape[0])
        self.batch_ids = np.arange(np.ceil(self.images.shape[0] / float(self.batch_size)), dtype=int)
