...
def get_club_coord(det_l, det_r, det_next, next=True, horiz=True):
    '''
    det_next -> det_r of the next bar for Horiz;
    det_next -> det_l of the next bar for Vert
    out:  C, CL, CU, CB
    '''
    if horiz:
        C = np.array(det_r)
...
croissant_times = [[[[] for k in range(60)] for j in range(2)] for i in range(3)]
for i_event, event in tqdm(enumerate(tchain), total=tchain.GetEntries()):

    if i_event > int(options.nEvents)-1: break
    # if i_event: break
    if i_event < 300000: continue
    if i_event > 400000: break
    ...
    for aTrack in event.Reco_MuonTracks:
        ...
        for DS_z in DS_zlist:
            ...
            DS_track_pos = list(aTrack.extrapolateToPlaneAtZ(DS_z))
...
## Croissant plots
croissant_hists = [[np.array([np.histogram(croissant_times[i][j][k], bins=20, range=(hist_low, hist_up))[0] for k in range(60)]) for j in range(2)] for i in range(3)]
print('\n\n', croissant_hists[1][1],'\n\n')
