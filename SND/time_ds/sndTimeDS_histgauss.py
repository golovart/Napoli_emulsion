...
for id0 in range(4):
    for id1 in range(2):
        print('X_shape:', croissant_hists[id0][id1][1].shape, 'Y_shape:', croissant_hists[id0][id1][2].shape, 'hist2d_shape:',croissant_hists[id0][id1][0].T.shape)
...
def gauss_fit(x, mu, sig, scale):
    return scale*np.exp(-(x-mu)**2/(2*sig*sig))/(np.sqrt(2*np.pi*sig*sig))


x_hist = (croissant_hists[id0][id1][1][1:] + croissant_hists[id0][id1][1][:-1])/2
mean_times = []
bar_steps = [0]
for i in range(4):
    for j in range(2):
        ...
        for b in range(0,120,2):
            #mu_gaus, std_gaus = scipy.stats.norm.fit((croissant_hists[id0][id1][0].T)[b])
            norm_hist = ((croissant_hists[id0][id1][0].T)[b] + 1e-7)/(np.max((croissant_hists[id0][id1][0].T)[b]) + 1e-7)
            par_opt, par_cov = curve_fit(gauss_fit, x_hist, norm_hist, p0=[-5.0, 0.5, 1.0])
            mean_times.append(par_opt[0] if np.abs(par_opt[0]+5)<5 else -5)
...
if not offset_corrected:
    ...
    np.savetxt('gaus_hist_offsets_ds.txt', median_times, fmt='%.3f')   

plt.step(chan_times, mean_times, alpha=0.5)
plt.grid()
plt.minorticks_on()
plt.ylim(-8,-3)
plt.title('Gauss hist times')
plt.savefig('gaus_hist_time_align_DS.png')
plt.show()


print('\nCorrected hist offsets: ', np.around(corr_off, decimals=3))
print('Corrected hist times spread:', np.around(np.std(corr_times[np.abs(corr_times)<1]), decimals=3))
...
plt.savefig('gaus_hist_corr_time_align_DS.png')
plt.show()
